...
public class BoardController {
	...
	public ResponseEntity<List<BoardDto>> listArticle(@ApiParam(value = "게시글을 얻기위한 부가정보.", required = true) BoardParameterDto boardParameterDto) throws Exception {
		logger.info("Board listArticle - 호출");
		System.out.println("나호출!!!!!!!");
		System.out.println(boardParameterDto.getPg());
		System.out.println(boardParameterDto.getSpp());
		System.out.println(boardParameterDto.getKey());
		System.out.println(boardParameterDto.getWord());
	...
	public ResponseEntity<?> getBoardTotalCount() throws Exception {
		logger.info("getBoardTotalCount - 호출");
		int result = 0;
		HttpStatus status = HttpStatus.ACCEPTED;
		result = boardService.getBoardTotalCount();
		System.out.println("공지사항 전체 수:>>>>" + result);
